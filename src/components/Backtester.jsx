...
export default function Backtester() {
  const { pairData, appSettings } = useAppContext();
  ...
  // Export CSV function – now inside the component
  const exportDataset = () => {
  ...
  const runBacktest = async () => {
    let closes;
    if (pairData[pair]?.closes) closes = pairData[pair].closes;
    else {
      try {
        if (appSettings.keys.twelve)
          closes = await fetchTwelveData(pair, appSettings.keys.twelve);
        else throw new Error("No historical data. Add Twelve Data key.");
      } catch (e) {
        alert(e.message);
        return;
      }
    ...
    const wins = trades.filter((t) => t.ret > 0),
      losses = trades.filter((t) => t.ret <= 0);
    const winRate = ((wins.length / trades.length) * 100).toFixed(1);
    const totalReturn = (equity[equity.length - 1] - 100).toFixed(2);
    const avgWin = wins.length
      ? ((wins.reduce((s, t) => s + t.ret, 0) / wins.length) * 100).toFixed(2)
      : "0.00";
    const avgLoss = losses.length
      ? ((losses.reduce((s, t) => s + t.ret, 0) / losses.length) * 100).toFixed(2)
      : "0.00";
    let peak = 100,
      maxDD = 0;
    equity.forEach((v) => {
    ...
    const sharpe =
      std > 0
        ? ((mean / std) * Math.sqrt(252 / rets.length)).toFixed(2)
        : "n/a";
    ...
    setTimeout(() => {
      ...
      if (finalAbove100) {
        grad.addColorStop(0, "rgba(61,184,124,0.25)");
        grad.addColorStop(1, "rgba(61,184,124,0)");
      } else {
        grad.addColorStop(0, "rgba(224,90,58,0.25)");
      ...
      ctx.strokeStyle = finalAbove100 ? "#3DB87C" : "#E05A3A";
      ctx.lineWidth = 2;
      ctx.stroke();
    ...
    <div className="page">
      <div className="section-head">
        ...
        <span className="section-sub">
          Define a rule-based strategy, run it on historical daily closes
        </span>
      </div>
        ...
        <div className="backtest-form">
          ...
          <div className="form-group">
            ...
            <select value={dir} onChange={(e) => setDir(e.target.value)}>
              <option value="long">Long (buy on entry)</option>
              <option value="short">Short (sell on entry)</option>
            </select>
          </div>
          <div className="form-group">
          ...
          <p style={{ fontSize: 11, color: "var(--color-text-faint)", lineHeight: 1.6 }}>Uses up to 2 years of daily closes from your saved API.</p>
        </div>
